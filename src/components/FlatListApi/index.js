...
class FlatListApi extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (failure && data.length > 0) {
      ...
      Util.showCustomMessage(errorMessage);
    }
  ...
  _sendRequest = (
    ...
    console.log("_sendRequest", data[data?.length - 1]);

    if (data.length > 8) {
      payloadAction.lastMessageObj = data[data?.length - 1];
    }
    if (successcb) {
      payloadAction.cb = successcb;
    }
  ...
  _renderEmptyView = () => {
    ...
    console.log(data, showOnly, emptyView);
